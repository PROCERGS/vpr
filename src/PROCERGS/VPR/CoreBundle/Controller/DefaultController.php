...
class DefaultController extends Controller
{
    ...
    public function endOfferAction(Request $request)
    {
        ...
        return compact(
            'nfgLink', 'loginCidadaoLink', 'skipModal'
        );
    }
    ...
    public function longPollingAction()
    {
        ...
        $url .= "/api/v1/wait/person/update?".http_build_query(array('access_token' => $accessToken,
                'updated_at' => $user->getLoginCidadaoUpdatedAt()->format('Y-m-d H:i:s')));
        try {
            ...
            return new JsonResponse(($received !== false && $received) ? $received
                        : null, $e->getCode());
        }
    ...
    {
        $limit     = ini_get('max_execution_time') ? ini_get('max_execution_time')
            - 2 : 60;
        $startTime = time();
        while ($limit > 0) {
