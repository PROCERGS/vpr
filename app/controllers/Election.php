...
class Election extends AppController {
	...
	public static function confirm() {
		...
		$captcha = self::getParam('recaptcha_response_field');
		if (isset($captcha))
			$votingSession->setCaptcha($captcha);
		...
		if (is_array($next_group) && reset($next_group) instanceof GrupoDemanda) {
			...
			$resp = recaptcha_check_answer($privatekey, $_SERVER["REMOTE_ADDR"], $_POST["recaptcha_challenge_field"], $captcha);

			
			//descomentar o $resp quando colocar em prod
			if ($resp->is_valid || isset($resp)) {
